...
$isEdit = $quiz !== null;
$storedQuestionIds = array_map('strval', array_column($quizQuestions ?? [], 'question_id'));
$selectedQuestions = array_map('strval', (array) old('question_ids', $storedQuestionIds));
$selectedMaterial = (string) old('material_id', $quiz['material_id'] ?? '');
$action = $isEdit ? site_url('admin/quizzes/' . $quiz['id']) : site_url('admin/quizzes');
$backUrl = $isEdit ? site_url('admin/quizzes/' . $quiz['id']) : site_url('admin/quizzes');
$settings = [
        ...
        <div class="grid items-start gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(22rem,.85fr)]">
            <div class="space-y-6">
                <section class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm" aria-labelledby="quiz-form-title">
                    <div class="question-form-heading"><p class="text-xs font-bold uppercase tracking-[.18em] text-orange-500"><?= $isEdit ? 'Edit Quiz' : 'New Quiz' ?></p><h2 id="quiz-form-title" class="mt-1 text-lg font-bold text-slate-800">Informasi Quiz</h2><p class="mt-1 text-xs text-slate-400">Tentukan materi, judul, durasi, dan standar kelulusan.</p></div>
                    <div class="p-5 md:p-6">
                        <div><label for="material_id" class="question-label">Material <span class="text-red-500">*</span></label><?php if ($isLocked): ?><input type="hidden" name="material_id" value="<?= esc($selectedMaterial, 'attr') ?>"><?php endif ?><select id="material_id" name="<?= $isLocked ? '' : 'material_id' ?>" class="question-control <?= isset($errors['material_id']) ? 'has-error' : '' ?>" required <?= $isLocked ? 'disabled' : '' ?>><option value="">Pilih material</option><?php foreach ($materials as $material): ?><option value="<?= $material['id'] ?>" <?= $selectedMaterial === (string) $material['id'] ? 'selected' : '' ?>><?= esc(($material['code'] ? $material['code'] . ' — ' : '') . $material['title']) ?></option><?php endforeach ?></select><?php if (isset($errors['material_id'])): ?><p class="question-error"><?= esc($errors['material_id']) ?></p><?php endif ?></div>
                        <div class="mt-5"><label for="title" class="question-label">Judul Quiz <span class="text-red-500">*</span></label><input id="title" name="title" type="text" value="<?= esc(old('title', $quiz['title'] ?? ''), 'attr') ?>" maxlength="200" class="question-control <?= isset($errors['title']) ? 'has-error' : '' ?>" placeholder="Contoh: Evaluasi Dasar HTML" required><?php if (isset($errors['title'])): ?><p class="question-error"><?= esc($errors['title']) ?></p><?php endif ?></div>
                        <div class="mt-5"><label for="description" class="question-label">Deskripsi</label><textarea id="description" name="description" rows="4" maxlength="5000" class="question-control resize-y <?= isset($errors['description']) ? 'has-error' : '' ?>" placeholder="Jelaskan tujuan atau petunjuk singkat quiz..."><?= esc(old('description', $quiz['description'] ?? '')) ?></textarea><?php if (isset($errors['description'])): ?><p class="question-error"><?= esc($errors['description']) ?></p><?php endif ?></div>
                        <div class="mt-5 grid gap-5 sm:grid-cols-3">
                            <div><label for="duration_minutes" class="question-label">Durasi <span class="text-red-500">*</span></label><div class="relative"><input id="duration_minutes" name="duration_minutes" type="number" value="<?= esc(old('duration_minutes', $quiz['duration_minutes'] ?? '15'), 'attr') ?>" min="1" max="480" class="question-control pr-16" required><span class="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 text-[.62rem] font-bold text-slate-400">MENIT</span></div><?php if (isset($errors['duration_minutes'])): ?><p class="question-error"><?= esc($errors['duration_minutes']) ?></p><?php endif ?></div>
                            <div><label for="passing_score" class="question-label">Nilai Lulus <span class="text-red-500">*</span></label><div class="relative"><input id="passing_score" name="passing_score" type="number" value="<?= esc(old('passing_score', $quiz['passing_score'] ?? '75'), 'attr') ?>" min="0" max="100" step="0.01" class="question-control pr-10" required><span class="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 text-[.7rem] font-bold text-slate-400">%</span></div><?php if (isset($errors['passing_score'])): ?><p class="question-error"><?= esc($errors['passing_score']) ?></p><?php endif ?></div>
                            <div><label for="status" class="question-label">Status <span class="text-red-500">*</span></label><select id="status" name="status" class="question-control" required><?php foreach (['DRAFT' => 'Draft', 'ACTIVE' => 'Aktif', 'INACTIVE' => 'Nonaktif'] as $value => $label): ?><option value="<?= $value ?>" <?= (string) old('status', $quiz['status'] ?? 'DRAFT') === $value ? 'selected' : '' ?>><?= $label ?></option><?php endforeach ?></select></div>
                        </div>
                    </div>
                </section>
                ...
                <section class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm" aria-labelledby="quiz-question-title">
                    ...
                    <div class="p-5 md:p-6">
                        <?php if ($isLocked): ?><div class="mb-4 rounded-lg border border-orange-200 bg-orange-50 px-3 py-2 text-xs leading-relaxed text-orange-700">Material dan susunan pertanyaan tidak dapat diubah karena quiz sudah memiliki riwayat pengerjaan.</div><?php endif ?>
                        ...
                        <div id="quiz-question-list" class="quiz-question-picker mt-4">
                            <?php foreach ($questions as $question): ?>
                                <label class="quiz-question-choice" data-material="<?= $question['material_id'] ?>" data-search="<?= esc(mb_strtolower($question['question_text']), 'attr') ?>">
                                    <input type="checkbox" name="question_ids[]" value="<?= $question['id'] ?>" class="quiz-question-checkbox" <?= in_array((string) $question['id'], $selectedQuestions, true) ? 'checked' : '' ?> <?= $isLocked ? 'disabled' : '' ?>>
                ...
                <section class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm" aria-labelledby="quiz-setting-title">
                    ...
                    <div class="divide-y divide-slate-100 px-5">
                        <?php foreach ($settings as $field => [$label, $description, $default]): ?>
                            <?php $checked = (string) old($field, (string) ($quiz[$field] ?? $default)) === '1'; ?>
