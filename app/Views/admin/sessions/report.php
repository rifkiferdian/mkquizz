...
<div class="mx-auto max-w-7xl">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <a href="<?= esc($reportContext['back_url'], 'attr') ?>" class="inline-flex items-center gap-2 text-xs font-semibold text-slate-400 transition hover:text-orange-600"><svg class="size-4" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="m15 18-6-6 6-6"/></svg><?= esc($reportContext['back_label']) ?></a>
    ...
    <section class="question-report-hero mt-5" aria-labelledby="question-report-title">
        <div class="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div><p class="text-xs font-bold uppercase tracking-[.18em] text-orange-200">Question Evaluation Report</p><h2 id="question-report-title" class="mt-2 text-2xl font-black tracking-tight text-white md:text-3xl">Evaluasi Jawaban Peserta</h2><p class="mt-2 max-w-3xl text-sm leading-relaxed text-orange-100">Analisis jawaban benar dan salah untuk menemukan soal yang perlu diperjelas atau materi yang perlu disampaikan kembali.</p><div class="mt-4 flex flex-wrap gap-2"><span class="rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-[.62rem] font-bold text-white"><?= esc($reportContext['scope_label']) ?></span><span class="rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-[.62rem] font-bold text-white"><?= esc($reportContext['quiz_title']) ?></span></div></div>
    ...
    <p class="mt-4 text-center text-[.62rem] leading-relaxed text-slate-400"><?= esc($reportContext['note']) ?> Satu peserta yang mengerjakan lebih dari sekali akan dihitung sesuai jumlah attempt.</p>
</div>
</div>
<?= $this->endSection() ?>
