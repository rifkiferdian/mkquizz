...
$statusValue = (string) old('status', 'WAITING');
$allowDuplicate = (string) old('allow_duplicate_name', '0') === '1';
?>
...
<div class="mx-auto max-w-7xl">
    ...
    <form action="<?= site_url('admin/sessions') ?>" method="post" class="mt-5">
        <?= csrf_field() ?>
        <div class="grid items-start gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(22rem,.85fr)]">
            <div class="space-y-6">
                <section class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm" aria-labelledby="session-form-title">
                    <div class="question-form-heading"><p class="text-xs font-bold uppercase tracking-[.18em] text-orange-500">New Session</p><h2 id="session-form-title" class="mt-1 text-lg font-bold text-slate-800">Informasi Sesi Quiz</h2><p class="mt-1 text-xs text-slate-400">Pilih quiz aktif dan tentukan identitas pelaksanaannya.</p></div>
                    <div class="p-5 md:p-6">
                        <div><label for="quiz_id" class="question-label">Quiz <span class="text-red-500">*</span></label><select id="quiz_id" name="quiz_id" class="question-control <?= isset($errors['quiz_id']) ? 'has-error' : '' ?>" required><option value="">Pilih quiz aktif</option><?php foreach ($quizzes as $quiz): ?><option value="<?= $quiz['id'] ?>" <?= $quizValue === (string) $quiz['id'] ? 'selected' : '' ?> <?= (int) $quiz['question_count'] === 0 ? 'disabled' : '' ?>><?= esc($quiz['title']) ?> — <?= (int) $quiz['question_count'] ?> soal</option><?php endforeach ?></select><p class="mt-1.5 text-[.65rem] text-slate-400">Quiz tanpa pertanyaan tidak dapat digunakan.</p><?php if (isset($errors['quiz_id'])): ?><p class="question-error"><?= esc($errors['quiz_id']) ?></p><?php endif ?></div>
                        <div class="mt-5"><label for="session_name" class="question-label">Nama Sesi <span class="text-red-500">*</span></label><input id="session_name" name="session_name" type="text" value="<?= esc(old('session_name'), 'attr') ?>" maxlength="200" class="question-control <?= isset($errors['session_name']) ? 'has-error' : '' ?>" placeholder="Contoh: Ujian HTML — Kelas XII RPL A" required><?php if (isset($errors['session_name'])): ?><p class="question-error"><?= esc($errors['session_name']) ?></p><?php endif ?></div>
                    </div>
                </section>
                ...
                <section class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm" aria-labelledby="session-schedule-title">
                    ...
                    <div class="p-5 md:p-6">
                        ...
                        <div class="mt-5 grid gap-5 sm:grid-cols-2">
                            <div><label for="max_participants" class="question-label">Maksimal Peserta</label><input id="max_participants" name="max_participants" type="number" value="<?= esc(old('max_participants'), 'attr') ?>" min="1" max="100000" class="question-control <?= isset($errors['max_participants']) ? 'has-error' : '' ?>" placeholder="Kosongkan jika tanpa batas"><?php if (isset($errors['max_participants'])): ?><p class="question-error"><?= esc($errors['max_participants']) ?></p><?php endif ?></div>
                            <div><label for="status" class="question-label">Status Awal <span class="text-red-500">*</span></label><select id="status" name="status" class="question-control <?= isset($errors['status']) ? 'has-error' : '' ?>" required><option value="DRAFT" <?= $statusValue === 'DRAFT' ? 'selected' : '' ?>>Draft</option><option value="WAITING" <?= $statusValue === 'WAITING' ? 'selected' : '' ?>>Menunggu</option><option value="OPEN" <?= $statusValue === 'OPEN' ? 'selected' : '' ?>>Langsung Dibuka</option></select><?php if (isset($errors['status'])): ?><p class="question-error"><?= esc($errors['status']) ?></p><?php endif ?></div>
                        </div>
                    </div>
                </section>
            </div>
            ...
            <div class="space-y-6 xl:sticky xl:top-24">
                <section class="session-create-access" aria-labelledby="automatic-access-title">
                    <div class="relative z-10"><div class="flex items-center justify-between"><div><p class="text-[.65rem] font-bold uppercase tracking-[.18em] text-orange-200">Automatic Access</p><h2 id="automatic-access-title" class="mt-1 text-base font-bold text-white">PIN & Token Otomatis</h2></div><svg class="size-7 text-orange-200" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="4" y="10" width="16" height="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg></div><div class="mt-6 rounded-xl border border-white/15 bg-white/10 p-4"><p class="text-[.62rem] uppercase tracking-wider text-orange-100">Contoh PIN</p><p class="mt-2 font-mono text-3xl font-black tracking-[.25em] text-white">••••••</p></div><p class="mt-4 text-[.68rem] leading-relaxed text-orange-100">Sistem membuat PIN 6 digit dan token unik setelah sesi berhasil disimpan.</p></div>
                </section>
